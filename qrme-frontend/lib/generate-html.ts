/**
 * Client-side HTML generator for QR template pages.
 * Mirrors the logic in app/api/generate-html/route.ts but runs in the browser.
 * Required because output:'export' strips Next.js API routes from the static build.
 */
...
export function generateHtml(profile: Profile, qr: QRData): string {
  ...
  if (socialEntries.length > 0) {
    const linksHtml = socialEntries.map(([platform, value]) => {
      ...
      return `<a href="${href}" target="_blank" rel="noopener noreferrer" style="display:inline-flex;align-items:center;gap:0.5rem;padding:0.5rem 1rem;border-radius:0.75rem;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);text-decoration:none;font-size:0.875rem;color:inherit;"><span>${icon}</span><span style="color:#a0a0b8;">${platform}</span></a>`;
    }).join('');
  ...
  if (profile.interests && profile.interests.length > 0) {
    const badges = profile.interests.map((i) =>
      `<span style="padding:0.25rem 0.75rem;border-radius:9999px;background:${theme.badgeBg};border:1px solid rgba(255,255,255,0.1);font-size:0.75rem;color:#d4d4e8;">${i}</span>`
    ).join('');
  ...
  const ageHtml = profile.age
    ? `<div style="position:absolute;bottom:-4px;right:-4px;background:${theme.accent};color:white;font-size:0.65rem;font-weight:700;border-radius:9999px;width:2rem;height:2rem;display:flex;align-items:center;justify-content:center;border:2px solid #1a1528;">${profile.age}</div>`
    : '';
